        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                    <h3 class="text-lg font-semibold text-gray-900">Formulir Pembaruan Barang</h3>
                    <p class="text-xs text-gray-500">Silakan ubah informasi barang di bawah ini.</p>
                </div>
                ...
                <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code', $product->code) }}" required placeholder="Contoh: BRG-001" class="w-full px-4 py-2.5 bg-gray-50 border-gray-200 focus:border-accent-500 focus:ring focus:ring-accent-500/20 rounded-xl text-sm transition-all duration-150 @error('code') border-rose-500 focus:border-rose-500 focus:ring-rose-200 @enderror">
                            @error('code')
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" required placeholder="Contoh: Laptop ASUS ROG" class="w-full px-4 py-2.5 bg-gray-50 border-gray-200 focus:border-accent-500 focus:ring focus:ring-accent-500/20 rounded-xl text-sm transition-all duration-150 @error('name') border-rose-500 focus:border-rose-500 focus:ring-rose-200 @enderror">
                            @error('name')
                                <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" required min="0" class="w-full px-4 py-2.5 bg-gray-50 border-gray-200 focus:border-accent-500 focus:ring focus:ring-accent-500/20 rounded-xl text-sm transition-all duration-150 @error('stock') border-rose-500 focus:border-rose-500 focus:ring-rose-200 @enderror">
                            @error('stock')
                                <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="location" id="location" value="{{ old('location', $product->location) }}" required placeholder="Contoh: Gudang A - Rak 2" class="w-full px-4 py-2.5 bg-gray-50 border-gray-200 focus:border-accent-500 focus:ring focus:ring-accent-500/20 rounded-xl text-sm transition-all duration-150 @error('location') border-rose-500 focus:border-rose-500 focus:ring-rose-200 @enderror">
                            @error('location')
                                <p class="text-xs text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-3">
                        ...
                        @if($product->image)
                            <div class="flex items-center space-x-4 p-3 rounded-xl border border-gray-100 w-fit">
                                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-16 h-16 object-cover rounded-lg">
                                <span class="text-xs text-gray-500">Gambar saat ini. Unggah baru untuk mengganti.</span>
                            </div>
                        @endif
